#!/bin/bash
#主机信息每日巡检
IPADDR=$(ifconfig ens33|grep 'inet'|awk '{print $2}')
HOST=$(hostname)
...
RESULTFILE="$LOGPATH/HostDailyCheck-$HOST-$IPADDR-`date +%Y%m%d`.txt"
#定义报表的全局变量
...
echo ""
echo "############################ 磁盘检查 ############################"
df -hiP -t xfs | sed 's/Mounted on/Mounted/'> /tmp/inode
df -hTP -t xfs | sed 's/Mounted on/Mounted/'> /tmp/disk 
...
#ip a
for i in $(ip link | grep BROADCAST | awk -F: '$2 !~ /veth/{print $2}');do ip add show $i | grep -E "BROADCAST|global"| awk '{print $2}' | tr '\n' ' ' ;echo "" ;done
fi
...
echo "############################ 文件检查 #############################"
workdir='/root'
check2=$(find $workdir -name '*.sh' -mtime -1)
check21=$(find $workdir -name '*.asp' -mtime -1)
check22=$(find $workdir -name '*.php' -mtime -1)
check23=$(find $workdir -name '*.aspx' -mtime -1)
check24=$(find $workdir -name '*.jsp' -mtime -1)
check25=$(find $workdir -name '*.html' -mtime -1)
check26=$(find $workdir -name '*.htm' -mtime -1)
check27=$(find $workdir -name '*.jar' -mtime -1)
check9=$(find $workdir -name core -exec ls -l {} \;)
...
echo -e "`date "+%Y-%m-%d %H:%M:%S"` 平台巡检报告"  | mail -a $RESULTFILE -s "平台巡检报告" sweet_love2000@126.com
