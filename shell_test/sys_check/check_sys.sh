...
# 查询进程
check_process(){
  process_list=("elastic" "zookeeper" "kafka" "mysql" "nginx" "java")
  check_num=0
...
check_space(){
  ...
  # 查询校验opt空间
  optspace=`df -h /home/linux | sed -n '2p' | awk -F' ' '{print $4}'|  awk -F'G' '{print $1}'`
  checkopt=`echo $optspace | grep -i 't'`
  if [[ $checkopt != "" ]]; then
    echo "磁盘空间为：$allspace G，/home/linux目录空间：$optspace，已满足安装条件"
  else
    if [[ $allspace -ge $disktotal ]]; then
      if [[ $optspace -ge $opttotal ]]; then
        echo "磁盘空间为：$allspace G，/home/linux目录空间：$optspace G，已满足安装条件"
      else
