...
process_list=(elastic zookeeper mysql nginx)
head_list=("主机名称" "IP" "操作系统" "系统内核" "系统语言" "系统编码" "当前时间" "启动时间" "运行时间")
...
hostname=$(hostname)
ip=$(hostname -i)
system=$(cat /etc/*-release|awk 'END{print}'|cut -d \= -f 2|sed 's/ /_/g')
core=$(uname -r|sed 's/ /_/g')
language=$(echo $LANG |awk -F "." '{print $1}')
code=$(echo $LANG |awk -F "." '{print $2}')
now=$(date +%F_%T)
up_time=$(who -b | awk '{print $3"_"$4}')
run_time=$(uptime | awk '{print $3$4}' | sed 's/,//g')

data_list=($hostname $ip $system $core $language $code $now $up_time $run_time)
...
iconv -f utf-8 -t gbk health.csv > health1.csv
