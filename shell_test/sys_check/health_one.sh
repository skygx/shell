...
MEM_THRESHOLD=80
DISK_THRESHOLD=90
#FILE_THRESHOLD=$(ulimit -n)$(ulimit -n)
...
if (( $(echo "$LOAD_AVERAGE > $LOAD_THRESHOLD" | bc -l) )); then
  ...
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到高系统负载平均值: $LOAD_AVERAGE\n"
fi
...
if (( $(echo "$CPU_USAGE > $CPU_THRESHOLD" | bc -l) )); then
  # 如果负载平均值超过阈值，则发送电子邮件警报
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到CPU使用率: $CPU_USAGE\n"
fi
...
if (( ${DISK_USAGE%?} >= $DISK_THRESHOLD )); then
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 在检测到高磁盘使用率: $DISK_USAGE\n"
fi
...
if (( $(echo "$MEMORY_USAGE > $MEM_THRESHOLD" | bc -l) )); then
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到高内存使用率: $MEMORY_USAGE%\n"
fi
...
if (( $(echo "$File_USAGE > $FILE_THRESHOLD" | bc -l) )); then
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到文件使用数: $File_USAGE\n"
fi
...
if (( $(echo "$NET_USAGE > $NET_THRESHOLD" | bc -l) )); then
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到网络连接数: $NET_USAGE\n"
fi
...
if (( $(echo "$INODE_USAGE > $INODE_THRESHOLD" | bc -l) )); then
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到INODE使用率: $INODE_USAGE\n"
fi
}
...
if [ $APP_STATUS -eq 0 ]; then
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到 $APP_NAME 进程不存. \n"
fi
...
if [ $PORT_STATUS -eq 0 ]; then
  HEALTH="FALSE"
  STATUS="$STATUS 在 $DATE 检测到 $PORT_NUM 端口号不存. \n"
fi
}
